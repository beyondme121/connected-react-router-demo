...
以前的流程操作是：初始化，mountComponent，new 渲染 Watcher，watcher中调用 vm._render来生成虚拟DOM, 将vdom传入到vm._update来生成真实的DOM, 内部调用patch方法，原理就是根据vnode，创建dom, 找到老dom的父节点，插入其中，删除老节点（也就是替换）。

**问题**：如果整个组件树种，100个属性，只有一个属性变更了，难道要把所有的dom节点全部删除然后再创建？

**解决**：我们希望在更新的时候，老的VDOM和新的VDOM进行对比， 将不同的vdom更新真实的dom

**问题**：为什么说，操作vdom的性能要优于操作dom呢？

**解决**：vdom是个js对象，属性是有限的，而真实的dom，有将近300个属性，每次创建dom酒要创建300个属性，性能不高。
...
**解决**：patch阶段：vdom/patch.js, 因为这个阶段是通过vdom来生成真实dom的阶段

patch：包含了 渲染 和 更新 功能



## 1. 模拟DOM-Diff更新过程

传渲染时，调用vm.$el = patch(vm.$el, vdom)，把整个虚拟DOM生成了真实DOM, 函数内部的vm.$el，就是真实的DOM, #app的DOM节点。

当更新时，继续调用render方法，生成新的vnode，把老的vnode和新的vnode传入到patch方法中进行比较。所以patch的第一个参数可能是真实DOM, 也可能是VDOM.

> 入口文件模拟patch，传入两个vnode进行模拟。
...
let vnode1 = render1.call(vm1)  // 返回虚拟DOM
...
patch(vnode1, vnode2) // 更新时，把新老vdom传入patch方法
```
...
patch的参数是真实DOM, 直接创建，并替换初始的#app
...
> 用新的虚拟节点，对比老的虚拟节点，找到差异，去更新老的DOM元素

**特点1**：在所有的diff算法中，只要是比较树tree，都是**同层平级比较**。为了比较方便。平级比较的时间复杂度就是On, 如果不是平级比较，复杂度就是O3，3次方。因为要遍历两棵树，同时更新还要遍历一次树。
...
因为文本的tag是undefined, 走到这里说明新老tag是一样相等的
...
1. 新老VDOM都有儿子(最复杂的真正意义上的dom-diff)
2. 老的有，新的没有，设置innerHTML为空
3. 老的没有，新的有，遍历新的VDOM的儿子数组，把每一个vdom变更dom，插入到当前节点中，即作为当前节点的child节点
...
```js
let render1 = compileToFunctions('<div id="a" style="color: red"></div>')
let render2 = compileToFunctions('<div id="b">{{name}}</div>')
setTimeout(() => {
  patch(oldVnode, newVnode) // 更新时，把新老vdom传入patch方法
}, 1500)
...
> 核心思想：双指针遍历
>
> 分别在两个Vnode的头，尾设置index，和VNODE，只要startIndex和endIndex相同了，之后循环就结束了。两个数组哪个数组先遍历完，就结束，然后分析是把多的那个数组的元素是增加到el元素上，还是删除老的。
>
> **遍历的满足条件：**只要有一个数组遍历完了，就停止另外一个数组的遍历，**把多余的元素放置在老的数组的前面或者后面**
>
> 1. oldVnode的startIndex <= endIndex  **并且 &&**
> 2. newVnode的startIndex <= endIndex 

##### 1. 优化范畴

> 对比新老vdom，相同就复用，或者移动dom；
...
老的尾和新的尾比较，如果相同，直接更新属性，如果当前的vnode也没有children，那就对比结束了，复用也就结束了
...
相同，属性更新，将老的头DOM节点，oldStartVnode.el **插入到老的尾部节点的下一个节点的前面**.

API: insertBefore(要插入的节点，期望插入的位置的前面)； 如果期望插入的位置为null，内部就是appendChild，如果有不是null，就直接插入到指定节点的前面。
...
> 儿子之间没有关系，暴力比对
>
> 1. 通过老的节点生成key和索引的mapping关系表。key-index关系。{A:0, B:1, C: 2}
> 2. 遍历新的，比对mapping关系，如果找不到，直接创建，并插入到老的开始索引的前面。新的数组索引++；
> 3. 如果匹配上了，说直接复用该节点，更新属性以及儿子节点，patch方法(包含了更新属性以及对比儿子children)
> 4. 老的数组开始index++，并且将当前匹配到的元素设置为null，为了避免数组塌陷。新的数据startIndex++，向后移动。
> 5. 知道新的数组遍历结束，然后重要的一点是，根据以新数组的startIndex开始遍历老的数组，将每个元素全部删除。
> 6. 如果老的数组指针移动到的元素是null，就直接跳过，继续对比下一个元素。

- 前提条件：把老节点的key与老节点的索引做成映射表 makeIndexByKey
...
let map = makeIndexByKey(oldChildren) // {A:0, B:1, C: 2}
```

- 暴力对比逻辑：遍历新数组查找老数组是否存在，存在复用和移动元素；不存在创建
...
```js
    ...
    if (findIndex == undefined) {
        ...
        patch(findVNode, newStartVnode) // 比较老的和新的VDOM的属性和儿子
...
- 遍历新数组时，当新元素在老的数组存在，会patch,更新属性和儿子，并继续走while循环，进行 头 ==> 头 比较， 会设置老的对应位置为null。oldChildren[findIndex] = null，这样，老的数组中，被命中的元素会被移动到startIndex的前面，同时startIndex也会向后移动，startIndex++，被命中的元素也被设置为了null，在while循环中，判断null和新数组中的元素比较是没有意义的，所以要跳过



##### 3. 简单总结

比较头()，尾，交叉(老的头和新的尾，老的尾和新的头)，如果都不相同，那就把老的数组做成一个key和index的映射表，拿新的数组的每一项到映射表中去查找，如果没有找到，就创建一个元素，插入到老的节点的前面。如果找到了，就复用即移动到指定的索引位置上。如果老的数组还没有遍历完，通过oldStartIndex小于oldEndIndex判断，那么把老的数组元素清空。parent.removeChild(child.el)



#### 5. 为什么要有key

循环的时候为什么不能用index作为key？DOM移动的性能要高于DOM创建
...
如果不是静态渲染的话，尽量不要使用index作为key。否则是没有元素复用的效果的。

比如，将列表倒叙，使用index作为key的场景分析：

> 元素li会复用, while循环里，进入了第一个判断，老的头和新的头比较，然后patch方法更新属性和文本内容，这里，文本内容发生了变更，要把老的文本内容重新赋值，就是重新替换。如果是个大的列表，要重新操作dom很多次，替换很多次文本内容。
>
> 不使用index作为key的场景：
>
> 通过isSameVnode判断两个节点是否为同一个节点，发现key不相同，不是同一个节点；跳过，走下一个流程；新的尾与老的尾比较，也不是同一个节点，跳过；老的头和新的尾部比较，发现tag 和 key 相同，是同一个节点，然后移动老的头部节点到老的尾部节点的下一个节点的前面，parent.insertBefore(oldStartVnode.el, oldEndVnode.nextSibling). 注意是 **移动！**

##### 

### 3. 修改调用处传递的参数

Vue初次渲染和更新都是调用了vm._update(), 其中patch()传递的第一个参数vm.$el是真实的DOM节点，这样总是会销毁DOM,创建DOM，走patch的第一个流程，没有实现DOM-DIFF的流程。
...
patch的第一个流程: 直接根据虚拟DOM创建真实DOM， 销毁老的dom。
...
那么如何区分是初次渲染还是更新呢？通过一个变量，如果初次渲染，这个变量没有值，渲染完成后，给变量赋值，第二次更新，再次判断这个变量有值了，就可以走到patch更新的DOM-DIFF流程分支了。
...
1. 组件中，不获取值的时候，computed中的方法会调用吗？ 不会。方法自己是不会执行的，只有你取值了，才会执行。
2. 取多次值，会多次调用吗？依赖的值不发生变化，就只执行一次。有缓存。控制着是否要执行内部有个变量，dirty
3. 取值就会执行，说明内部是getter，使用了Object.defineProperty.
4. 当依赖的值发生变化时，会让计算属性重新计算，控制着dirty 为true还是false。隐含着依赖收集，computed还是一个watcher，computed内部依赖的属性会收集这个watcher。比如firstName会收集这个watcher，lastName也会收集
...
- 初始化computed
- 定义每一个computed，将计算属性挂载到实例上，defineComputed，为后续初始化watcher
...
- 处理每个computed的get, set；当get时，收集依赖，set时触发watcher，重新调用getter执行。
...
> **问题**
>
> 1. 当我每次取值时，都调用了userDef.get, 每次取值都调用，没有缓存。
> 2. 当依赖的属性变了，还需要有一个watcher

## 3. 缓存

逻辑是：把函数的get方法做成一个**高阶函数**，在高阶函数内部根据变量dirty，来控制是否执行userDef函数。

只有取值的时候才有缓存的概念，设置值就直接调用set方法。

将userDef包装成**高阶函数**： createComputedGetter(userDef)
...
function initComputed(vm) {
  ...
  // 因为依赖的属性变更，计算属性要重新执行, 有依赖收集的功能, 内部实现了watcher，所以要重写computed的每一个属性,
  // 并且将属性写到vm实例上，可以让组件通过this.计算属性获的形式 获取到这个值
  for (let key in computed) {
    const userDef = computed[key]     // 两种计算属性的写法 1. 函数 2. 对象
...
- 给每个计算属性进行defineProperty时，get取值的时候，判断是否为dirty的，如果是dirty的，就执行getter重新计算取值，否则什么也不做。使用高阶函数进行切片编程，增加附加的功能，依赖收集。
...
function defineComputed (target, key, userDef) {
    ...
    if (typeof userDef == 'function') {
        sharedPropertyDefinition.get = createComputedGetter(key) // userDef
    } else {
        sharedPropertyDefinition.get = createComputedGetter(key)  // userDef.get
        sharedPropertyDefinition.set = userDef.set
    }
...
- 定义createComputedGetter, 作用是: 包装计算属性的get的函数定义，看看当前计算属性的watcher是否是dirty，如果是dirty的，执行get方法，否则不执行函数，直接返回第一次取值返回的watcher实例上的value。结果watcher.value
...
> 下面复习一下watcher.get方法.
>
> 1. 将当前的Watcher实例watcher，就是计算属性中new 的watcher放在 Dep的实例上。是哪个dep呢? 计算属性的依赖属性，比如firstName, lastName
> 2. 调用this.getter(), 就是执行计算属性对应的函数定义，即取值操作，就会对依赖的属性进行取值操作. return this.firstName + this.lastName。
> 3. 流程就会走到observer中的Object.defineProperty的get逻辑。因为已经pushTarget(this), 就会依赖收集dep.depend()，就会调用，dep和watcher的双向记忆
>
> ```js
> // observer/watcher.js
> get() {
...
> ```
>
> 4. dep.depend()就会调用watcher实例收集依赖。双向记忆。watcher记录依赖的dep，当前dep记录往前的watcher
>
> ```js
> class Dep {
>     depend() {
...
> ```
>
> 5. watcher.addDep(dep): watcher.deps: watcher依赖的所有deps，记录当前的dep, 比如firstName; 并且让当前的dep记录watcher。
>
> ```js
> // watcher记录dep，去重
> addDep(dep) {
...
>     let id = dep.id
>     if (!this.depsId.has(id)) {
>         this.deps.push(dep)		// watcher记录当前的dep
>         this.depsId.add(id)
>         dep.addSub(this)		// dep记录watcher
>     }
> }
...
- computed默认是不执行的，在watcher的构造函数中，判断watcher的类型，如果是computed，什么也不做void 0；
...
> index.html中，不获取computed，不会执行。

- 不取值, 不会调用计算属性的get方法，get方法由createComputedGetter(key)定义，高阶函数, 取值才会获取对应的watch，取evaluate。由于初始化计算属性的watcher时，lazy是true，所以不对计算属性computed求值，即不调用this.get()

- 多次取值，缓存的功能实现了, 第一次取值, 进行evaluate，并返回值；第二次，dirty为false，直接返回watcher.value
...
- 流程会走到Object.defineProperty(vm, key, {set () {}}) set流程中, Object.defineProperty, 调用dep.notify()
...
- dep.notify(), 执行所有的watcher的update方法。this.subs保存的是当前compute中的options中的当前key的所有watcher
...
- **watcher 的 update。因为依赖属性发生了变更，需要重新执行watcher。需要判断一下是否为computed, 设置为脏的。因为属性发生了变更，比如firstName发生了变更，是一定要重新渲染页面的。当再次获取这个计算属性的时候，get的时候，就重新计算这个计算属性，就得到了最新的值。**
...
- 测试. 
  ...
  - 多次取值：使用缓存
  - 修改了依赖属性，不获取computed的值：computed不执行！
...
setTimeout(() => {
    vm.firstName = 100
    // vm.fullName
    console.log(vm.fullName)		// 再次获取值, 就会重新计算
}, 1000)
```

但是：获取不到这个计算属性的值，结果为NaN。
...
1. 此处的this.getter()中的this是Watcher类的实例watcher。但是计算属性定义中的computed的函数定义在了组件或者vue实例上
2. 所以，修改this.getter的this执行 **this.getter.call(this.vm)**



## 5. 页面渲染问题

- 页面中使用计算属性，当计算属性依赖的值发生变化了，**页面却没有更新！**
...
1. 页面取值时，{{fullName}}，会调用computed的对应方法, 这样，firstName,lastName会收集 computed的watcher
2. 但是firstName,lastName并没有收集渲染watcher，只收集了计算属性的watcher。

解决：

在firstName,lastName收集计算属性的watcher的同时，把渲染watcher也进行收集。使用栈型结构去收集。把所有的watcher都收集好了，再自动调用。依赖属性或者data在收集watcher的时候，都保存到栈中。



思路：

当页面取值时, {{fullName}}, 页面会先进行渲染，就会有一个**渲染watcher**，然后又会取计算属性，又会有一个**计算属性的watcher**

将两个watcher存入到stack中。当取值时，会调用计算属性的get方法，就会让依赖的属性取值，取值就会收集依赖。让当前的watcher记录依赖属性，dep.depend(), => Dep.target.addDep(this); 记住当前的属性。当前的Dep.target就是计算属性watcher。然后就会触发双向记录。
...
addDep(dep) {
    ...
    let id = dep.id
    if (!this.depsId.has(id)) {
        this.deps.push(dep)	// 当前watcher记录依赖项
        this.depsId.add(id)
        dep.addSub(this)	// 当前依赖项记住当前的计算属性watcher
    }
}
```
...
1. **使用栈结构来保存所有的watcher，包含了渲染和计算属性watcher**
...
2. 栈中的结构如下图：两个watcher。fullName计算属性有两个依赖，这两个依赖项就把当前的计算属性watcher收集起来了。firstName, lastName 收集了 fullName watcher, 当前的watcher也收集了两个依赖属性。收集完毕后(双向记忆)后，就把stack出栈了。popTarget()。stack中只有渲染watcher了，同时两个依赖属性也记录计算属性watcher。以上只是增加了一个数组，而不是以前的单变量的处理方式，只能处理一个watcher。
3. 此时Dep.target指向了渲染watcher, Dep.target = stack[stack.length - 1]
4. 判断，如果Dep.target有值，就把这个Dep.target这个渲染watcher也放到依赖属性的watcher上就可以了。此时，计算属性的依赖项，就保存的计算属性watcher和渲染watcher。
5. 那么，当依赖项发生了变化，就让记录在依赖项的watcher数组中的所有watcher执行即可。



<img src="2020-08-24-13-20-12.png" style="zoom:80%;" />
...
```js
// state.js
// 此方法是我们包装的方法，每次取值会调用此方法  // 取值时才会调用
function createComputedGetter(key) {
  return function () {
    const watcher = this._computedWatchers[key] // 拿到这个属性对应的watcher
    if (watcher) {
      ...
      return watcher.value // 如果不是dirty的，说明是新的，赶紧的，直接返回
    }
  }
}
```
...
- watcher.js, 所有的依赖属性收集渲染watcher，调用dep.js中的depend方法
...
- this.deps[i].depend()就会执行 dep.js中的depend，内部的Dep.target就是出栈后的watcher, 渲染watcher。加上依赖。
...
3. 编写组件能拆分尽量拆分 vue的更新问题 每个组件一个watcher 可以减少比对。当数据发生变化，就会触发watcher进行update，进行dom-diff比对。如果拆分成小的组件，只需要找到变更的小组件进行dom-diff更新即可。更新范围就会减小。
